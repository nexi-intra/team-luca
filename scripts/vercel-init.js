...
function isVercelLinked() {
  try {
    const projectInfo = execSync('vercel project ls --json', { encoding: 'utf-8' });
...
function pullEnvVars() {
  try {
    ...
    console.log('✅ Successfully created .env.local file with Vercel environment variables');
...
function main() {
  ...
  if (!isVercelLinked()) {
    ...
    console.log('2. Follow the prompts to link to an existing project or create a new one');
    console.log('3. Once linked, run: npm run vercel:init\n');
    console.log('For more information, visit: https://vercel.com/docs/cli/link');
    process.exit(1);
  }
