...
function processFile(filePath) {
  ...
  for (const special of specialCases) {
    if (special.before.test(content)) {
      ...
      changes.push(
        `Applied special case: ${special.before} -> ${special.after}`,
      );
    }
  }
  ...
  for (const rule of updateRules) {
    ...
    if (matches) {
      ...
      changes.push(
        `Updated ${matches.length} import(s) matching ${rule.pattern}`,
      );
    }
  }
...
async function main() {
  ...
  for (const file of files) {
    ...
    const libImports = content.match(
      /from\s+['"]@\/lib\/(logger|config|telemetry|types|utils|auth|features)/g,
    );
    if (libImports) {
