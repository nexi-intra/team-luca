...
const SECRET_KEYWORDS = [
  ...
  'credential', 'private', 'api_key', 'apikey', 'access_token',
  'refresh_token', 'client_secret', 'encryption', 'salt', 'hash',
  'signature', 'certificate', 'cert', 'pem', 'ssh', 'gpg', 'pgp'
];
...
function checkGitHubEnvVar(repo, key, isSecret) {
  ...
  try {
    const output = execSync(`gh ${listCommand} -R ${owner}/${repoName} --json name`, {
      encoding: 'utf-8',
...
function setGitHubEnvVar(repo, key, value, isSecret) {
  ...
  if (!forceUpdate && checkGitHubEnvVar(repo, key, isSecret)) {
    console.log(`⏭  Skipping existing ${type}: ${key} (use --force to update)`);
    return;
  }
  ...
  try {
    ...
    if (isSecret) {
      execSync(`echo -n "${value}" | gh ${command} ${key} -R ${owner}/${repoName}`, {
        encoding: 'utf-8',
...
async function main() {
  ...
  console.log(`Detected ${secrets.length} secrets and ${variables.length} variables.\n`);
