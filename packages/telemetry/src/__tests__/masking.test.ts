...
describe("Masking Utilities", () => {
  describe("maskSensitiveValue", () => {
    it("should mask email addresses", () => {
      ...
      expect(maskSensitiveValue("john.doe+test@company.org")).toBe(
        "***@company.org",
      );
    });
  ...
  describe("sanitizeUrl", () => {
    it("should mask sensitive query parameters", () => {
      const url =
        "https://api.example.com/endpoint?apiKey=secret&user=john&password=pass123";
    ...
    it("should handle complex query parameters", () => {
      const url =
        "https://api.example.com?token=abc123&data[secret]=hidden&normal=value";
      const sanitized = sanitizeUrl(url);
