...
describe('OperationMetrics', () => {
  ...
  describe('increment', () => {
    ...
    it('should track operations with different attributes separately', () => {
      ...
      expect(OperationMetrics.getCount(OPERATION_KEYS.API_REQUEST, attributes1)).toBe(2);
      expect(OperationMetrics.getCount(OPERATION_KEYS.API_REQUEST, attributes2)).toBe(1);
    });
  ...
  describe('getCount', () => {
    it('should return 0 for non-existent operations', () => {
      expect(OperationMetrics.getCount(OPERATION_KEYS.API_REQUEST)).toBe(0);
      expect(OperationMetrics.getCount(OPERATION_KEYS.AUTH_LOGIN, { method: 'oauth' })).toBe(0);
    });
  ...
  describe('reset', () => {
    it('should clear all counters', () => {
      ...
      OperationMetrics.increment(OPERATION_KEYS.UI_INTERACTION, { component: 'button' });
      
      expect(OperationMetrics.getCount(OPERATION_KEYS.API_REQUEST)).toBe(1);
      expect(OperationMetrics.getCount(OPERATION_KEYS.AUTH_LOGIN)).toBe(1);
      expect(OperationMetrics.getCount(OPERATION_KEYS.UI_INTERACTION, { component: 'button' })).toBe(1);
      
      OperationMetrics.reset();
      
      expect(OperationMetrics.getCount(OPERATION_KEYS.API_REQUEST)).toBe(0);
      expect(OperationMetrics.getCount(OPERATION_KEYS.AUTH_LOGIN)).toBe(0);
      expect(OperationMetrics.getCount(OPERATION_KEYS.UI_INTERACTION, { component: 'button' })).toBe(0);
    });
  });
  ...
  describe('getAll', () => {
    it('should return all counters', () => {
      OperationMetrics.increment(OPERATION_KEYS.API_REQUEST);
      OperationMetrics.increment(OPERATION_KEYS.API_REQUEST);
      OperationMetrics.increment(OPERATION_KEYS.AUTH_LOGIN, { method: 'password' });
      
      const all = OperationMetrics.getAll();
      
      expect(all.size).toBe(2);
      expect(all.get(`${OPERATION_KEYS.API_REQUEST}:{}`)).toBe(2);
      expect(all.get(`${OPERATION_KEYS.AUTH_LOGIN}:{"method":"password"}`)).toBe(1);
    });
