...
describe('ConfigHelpers', () => {
  ...
  describe('getRequired', () => {
    ...
    it('should throw when value does not exist', () => {
      ...
      expect(() => helpers.getRequired('api.token')).toThrow('Required configuration not found: api.token');
      expect(() => helpers.getRequired('nonexistent')).toThrow('Required configuration not found: nonexistent');
    });
  });
  ...
  describe('getOrDefault', () => {
    ...
    it('should return default when value does not exist', () => {
      ...
      expect(helpers.getOrDefault('api.token', 'default-token')).toBe('default-token');
  ...
  describe('getValidated', () => {
    ...
    it('should throw when validation fails with boolean', () => {
      ...
      expect(() => helpers.getValidated('app.port', validator))
        .toThrow('Validation failed for configuration: app.port');
    });
    ...
    it('should throw with custom message when validation fails', () => {
      ...
      const validator = (value: number) => value > 5000 ? true : 'Port must be greater than 5000';
      expect(() => helpers.getValidated('app.port', validator))
        .toThrow('Port must be greater than 5000');
    });
