...
describe('EnvConfigProvider', () => {
  ...
  describe('loadConfiguration', () => {
    ...
    it('should parse array values from comma-separated strings', () => {
      process.env.API_ENDPOINTS = 'http://api1.com,http://api2.com,http://api3.com';
    ...
    it('should parse JSON objects', () => {
      process.env.API_HEADERS = '{"Authorization": "Bearer token", "X-Custom": "value"}';
  ...
  describe('validate', () => {
    it('should validate required fields', () => {
      ...
      expect(result.errors).toContain('API Key (api.key) is required but not set');
    });
    ...
    it('should run custom validators', () => {
      ...
      process.env.PORT = '80';
      const provider = new EnvConfigProvider(schemaWithValidator, { 'app.port': { envVar: 'PORT' } });
      ...
      if (result.valid) {
        ...
        expect(result.errors[0]).toContain('Port must be between 1024 and 65535');
      }
    });
  });
