...
describe("ScriptManager", () => {
  ...
  describe("executeScript", () => {
    ...
    it("should prevent duplicate script execution", async () => {
      ...
      await expect(scriptManager.executeScript(testScript)).rejects.toThrow(
        `Script ${testScript.id} is already running`,
      );
    ...
    it("should handle missing script", async () => {
      ...
      await expect(scriptManager.executeScript(testScript)).rejects.toThrow(
        `Script not found: ${testScript.name}`,
      );
  ...
  describe("stopScript", () => {
    it("should stop a running script", async () => {
      ...
      expect(mockIO.emit).toHaveBeenCalledWith("script:stopped", {
        id: testScript.id,
      });
    ...
    it("should throw error when stopping non-running script", () => {
      expect(() => scriptManager.stopScript("non-existent")).toThrow(
        "Script non-existent is not running",
      );
