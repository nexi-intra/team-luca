...
export const customAuthConfig: AuthConfig = {
  ...
  authority: config.getOrDefault('auth.authority', 'https://login.microsoftonline.com/common'),
  redirectUri: config.getOrDefault('auth.redirectUri', config.getOrDefault('general.appUrl', 'http://localhost:3000')),
  postLogoutRedirectUri: config.getOrDefault('auth.postLogoutRedirectUri', config.getOrDefault('general.appUrl', 'http://localhost:3000')),
  scopes: config.getOrDefault('auth.scopes', ['openid', 'profile', 'email', 'offline_access', 'User.Read']),
  popup: {
    width: 500,
    height: 600
  },
...
export function isAuthConfigured(): boolean {
  ...
  if (provider === 'supabase') {
    return !!(
      config.get('auth.supabaseUrl') &&
      config.get('auth.supabaseAnonKey')
    );
  }
