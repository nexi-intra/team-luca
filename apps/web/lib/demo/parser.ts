  ...
  for (const line of lines) {
    ...
    if (currentSection === 'metadata') {
      ...
      if (match) {
        ...
        else if (key === 'tags') script.metadata!.tags = value.split(',').map(t => t.trim());
...
function parseStepBlock(block: string): DemoStep | null {
  ...
  for (const line of lines) {
    ...
    const typeMatch = trimmed.match(/^(click|type|wait|navigate|assert|highlight|scroll|hover)\s*(.*)$/i);
    if (typeMatch) {
...
export function generateMarkdownScript(script: DemoScript): string {
  ...
  if (Object.keys(script.metadata || {}).length > 0 || script.id) {
    ...
    if (script.metadata?.author) markdown += `- author: ${script.metadata.author}\n`;
    if (script.metadata?.version) markdown += `- version: ${script.metadata.version}\n`;
    if (script.metadata?.tags) markdown += `- tags: ${script.metadata.tags.join(', ')}\n`;
    markdown += '\n';
  }
