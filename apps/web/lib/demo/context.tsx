...
function demoReducer(state: DemoState, action: DemoAction): DemoState {
  switch (action.type) {
    ...
    case "ADD_HIGHLIGHT":
      return {
        ...state,
        highlights: [...new Set([...state.highlights, action.elementId])],
      };
    case "REMOVE_HIGHLIGHT":
      return {
        ...state,
        highlights: state.highlights.filter((id) => id !== action.elementId),
      };
...
export function DemoProvider({ children }: DemoProviderProps) {
    ...
    async (step: DemoStep) => {
      ...
      try {
        switch (step.type) {
          ...
          case "type": {
            ...
            for (const char of step.value!) {
              ...
              await new Promise((resolve) =>
                setTimeout(resolve, 50 / state.speed),
              );
            }
          ...
          case "wait": {
            await new Promise((resolve) =>
              setTimeout(resolve, (step.delay || 1000) / state.speed),
            );
            break;
          }
          ...
          case "highlight": {
            ...
            dispatch({ type: "ADD_HIGHLIGHT", elementId: step.target! });
            await new Promise((resolve) =>
              setTimeout(resolve, (step.delay || 2000) / state.speed),
            );
            break;
          }
          ...
          case "scroll": {
            ...
            await new Promise((resolve) =>
              setTimeout(resolve, 1000 / state.speed),
            );
            break;
          }
  ...
  const nextStep = useCallback(() => {
    ...
    const nextIndex = Math.min(
      ...
      state.currentScript.steps.length - 1,
    );
  ...
  const goToStep = useCallback(
    (index: number) => {
      ...
      const validIndex = Math.max(
        0,
        Math.min(index, state.currentScript.steps.length - 1),
      );
