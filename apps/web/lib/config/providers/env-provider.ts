import { EnvConfigProvider as BaseEnvConfigProvider, ConfigCategory } from '@monorepo/config';
...
export class EnvConfigProvider extends BaseEnvConfigProvider<AppConfig> {
  ...
  validate(): { valid: boolean; errors: string[] } {
    ...
    errors.push(...baseResult.errors.filter(error => !error.includes('auth.')));
    ...
    if (provider === 'entraid') {
      if (!this.get('auth.clientId')) {
        errors.push('Microsoft Entra ID Client ID (auth.clientId) is required when using Entra ID authentication');
      }
      if (!this.get('auth.authority')) {
        errors.push('Microsoft Entra ID Authority (auth.authority) is required when using Entra ID authentication');
      }
      if (!this.get('auth.sessionSecret')) {
        errors.push('Session Secret (auth.sessionSecret) is required when using Entra ID authentication');
      }
    }
    
    if (provider === 'supabase') {
      if (!this.get('auth.supabaseUrl')) {
        errors.push('Supabase URL (auth.supabaseUrl) is required when using Supabase authentication');
      }
      if (!this.get('auth.supabaseAnonKey')) {
        errors.push('Supabase Anon Key (auth.supabaseAnonKey) is required when using Supabase authentication');
      }
      if (!this.get('auth.sessionSecret')) {
        errors.push('Session Secret (auth.sessionSecret) is required when using Supabase authentication');
      }
    }
    ...
    for (const key in authConfig) {
      ...
      if (['clientId', 'authority', 'supabaseUrl', 'supabaseAnonKey', 'sessionSecret'].includes(key)) {
        continue;
      }
