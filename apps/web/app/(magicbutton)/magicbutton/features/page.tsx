...
export default function FeaturesPage() {
  ...
  const filteredFeatures =
    selectedRing === "all" ? allFeatures : getFeaturesByRing(selectedRing);
    ...
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2 text-foreground">
          Feature Rings & Access
        </h1>
        <p className="text-muted-foreground">
          Manage your feature access level and explore available features across
          different rings.
        </p>
      </div>
        ...
        <CardContent>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div
                className={cn("w-4 h-4 rounded-full", ringColors[userRing])}
              ></div>
              <div>
                <p className="font-medium">
                  Ring {userRing} - {getRingName(userRing)}
                </p>
                <p className="text-sm text-muted-foreground">
                  {getRingDescription(userRing)}
                </p>
              </div>
            </div>
            <Select
              ...
              onValueChange={(value) =>
                handleRingChange(parseInt(value) as FeatureRing)
              }
            >
                    ...
                    <div className="flex items-center gap-2">
                      <div
                        className={cn(
                          ...
                          ringColors[ring as FeatureRing],
                        )}
                      ></div>
                      <span>
              ...
              <CardContent>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm">
                    {featuresInRing.length} features
                  </span>
                </div>
              </CardContent>
            </Card>
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <Select
          value={selectedRing.toString()}
          onValueChange={(value) =>
            setSelectedRing(
              value === "all" ? "all" : (parseInt(value) as FeatureRing),
            )
          }
        >
          <SelectTrigger className="w-48">
      ...
      {filteredFeatures.length === 0 && (
        <div className="text-center py-12">
          <p className="text-muted-foreground">
            No features found for the selected ring.
          </p>
        </div>
      )}
    </div>
