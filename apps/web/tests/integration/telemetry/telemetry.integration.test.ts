...
describe("OpenTelemetry Integration", () => {
  test("should capture server-side traces", async () => {
    ...
    const traces = await helpers.waitForTraces(
      "magic-button-assistant",
      undefined,
      1,
    );
    ...
    // Find HTTP span
    const httpSpan = trace.spans.find(
      (s) =>
        s.operationName.includes("GET") ||
        s.operationName.includes("/api/health"),
    );
  ...
  test("should mask sensitive data in traces", async () => {
    ...
    await axios
      .post("http://localhost:3000/api/test-sensitive", sensitiveData, {
        headers: {
          "x-request-id": helpers.getRunId(),
    ...
    for (const trace of traces) {
      ...
      for (const span of spans) {
        ...
        if (attrs["http.request.headers.authorization"]) {
          expect(attrs["http.request.headers.authorization"]).toBe(
            "Bearer ***",
          );
        }
...
describe("Telemetry Configuration", () => {
  test("should respect sampling rate configuration", async () => {
    ...
    logger.info(
      "Sampling rate test placeholder - would verify sampling behavior",
    );
  });
});
