...
test.describe("Feature Ring System", () => {
  ...
  test("should display features based on user ring level", async ({
    authenticatedPage,
    testUser,
  }) => {
  ...
  test("should persist feature ring selection", async ({
    authenticatedPage,
  }) => {
  ...
  test("should update UI when ring selection changes", async ({
    authenticatedPage,
  }) => {
    await featureHelper.setFeatureRing(4);
    await featureHelper.expectFeatureEnabled("stable-feature");
  ...
  test("should handle feature gates correctly", async ({
    authenticatedPage,
  }) => {
    await featureHelper.setFeatureRing(3);

    const betaFeature = await authenticatedPage.$(
      '[data-feature-id="beta-feature"]',
    );
    expect(betaFeature).toBeTruthy();

    const experimentalFeature = await authenticatedPage.$(
      '[data-feature-id="experimental-feature"]',
    );
    expect(experimentalFeature).toBeFalsy();
  });

  test("should show disabled features with reduced opacity when configured", async ({
    authenticatedPage,
  }) => {
    await featureHelper.setFeatureRing(4);

    const experimentalElement = await authenticatedPage.$(
      '[data-feature-id="experimental-feature"][data-show-when-disabled="true"]',
    );
...
test.describe("Feature Ring Integration", () => {
  ...
  test("should handle multi-feature gates", async ({ authenticatedPage }) => {
    ...
    const multiFeatureElement = await authenticatedPage.$(
      '[data-multi-feature-gate="true"]',
    );

    if (multiFeatureElement) {
      const requiredFeatures = await multiFeatureElement.getAttribute(
        "data-required-features",
      );
      const requireAll =
        await multiFeatureElement.getAttribute("data-require-all");
