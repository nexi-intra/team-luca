...
export class AuthHelper {
  ...
  async mockLogin(user: { email: string; name: string; id: string }) {
    await this.page.evaluate((userData) => {
      ...
      sessionStorage.setItem('msal.token.keys', JSON.stringify({
        idToken: [`${userData.id}.login.windows.net-idtoken-test-client-id-test-tenant-id-`],
        accessToken: [`${userData.id}.login.windows.net-accesstoken-test-client-id-test-tenant-id-openid profile`],
        refreshToken: [`${userData.id}.login.windows.net-refreshtoken-test-client-id-`],
      }));
      ...
      sessionStorage.setItem('msal.account.keys', JSON.stringify([userData.id]));
      sessionStorage.setItem(userData.id, JSON.stringify(mockAuthResponse.account));
    }, user);
  }
  ...
  async logout() {
    ...
    await this.page.waitForSelector('[data-testid="login-button"]', { timeout: 5000 });
  }
