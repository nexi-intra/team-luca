...
export const CONFIG_SCHEMA: AppConfig = {
  auth: {
    provider: {
      ...
      validate: (value) => {
        ...
        return validProviders.includes(value) || `Must be one of: ${validProviders.join(', ')}`;
      }
    },
    clientId: {
      value: '',
      name: 'Client ID',
      description: 'Microsoft Entra ID application (client) ID for authentication',
      featureRing: FeatureRing.GA,
    ...
    authority: {
      value: '',
      name: 'Authority URL',
      description: 'Microsoft identity platform authority URL (tenant-specific or common)',
      featureRing: FeatureRing.GA,
      required: false, // Now optional, only required for entraid
      ...
      validate: (value) => {
        ...
        return urlRegex.test(value) || 'Must be a valid Microsoft authority URL';
      }
    },
    redirectUri: {
      value: '',
    ...
    sessionSecret: {
      value: '',
      name: 'Session Secret',
      description: 'Secret key for encrypting session data (minimum 32 characters)',
      featureRing: FeatureRing.GA,
      required: true,
      example: 'your-super-secret-session-key-make-it-very-long',
      validate: (value) => value.length >= 32 || 'Must be at least 32 characters long'
    },
    scopes: {
      value: [],
      name: 'OAuth Scopes',
  ...
  api: {
    anthropicKey: {
      ...
      validate: (value) => value.startsWith('sk-ant-') || 'Must start with sk-ant-'
    }
  },
  ...
  general: {
    ...
    environment: {
      ...
      validate: (value) => ['development', 'production', 'test'].includes(value) || 'Must be development, production, or test'
    },
    port: {
      value: 2803,
      name: 'Server Port',
      ...
      defaultValue: 2803,
      example: '3000',
      validate: (value) => value > 0 && value <= 65535 || 'Must be a valid port number (1-65535)'
    }
  },
  ...
  telemetry: {
    ...
    logLevel: {
      ...
      required: false,
      defaultValue: 'INFO',
      validate: (value) => ['VERBOSE', 'INFO', 'WARN', 'ERROR', 'NONE'].includes(value) || 'Must be VERBOSE, INFO, WARN, ERROR, or NONE'
    },
    clientLogLevel: {
      value: 'INFO',
