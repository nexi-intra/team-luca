...
const consoleFormat = winston.format.combine(
  ...
  winston.format.printf(({ timestamp, level, message, source, ...metadata }) => {
    const shortLevel = levelDisplay[level] || level.toUpperCase().substring(0, 3);
...
export function createLogger(source) {
  return {
    verbose: (message, metadata = {}) => logger.verbose(message, { source, ...metadata }),
    info: (message, metadata = {}) => logger.info(message, { source, ...metadata }),
    warn: (message, metadata = {}) => logger.warn(message, { source, ...metadata }),
    error: (message, metadata = {}) => logger.error(message, { source, ...metadata }),
  };
}
        ...
        if (ioInstance) {
          const logEntry = {
            ...
            metadata: Object.keys(info)
              .filter(key => !['timestamp', 'level', 'message', 'source', 'service'].includes(key))
