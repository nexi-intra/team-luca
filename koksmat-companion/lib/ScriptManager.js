...
export class ScriptManager {
  ...
  async executeScript({ id, name, args = [], env = {} }) {
    ...
    return new Promise((resolve, reject) => {
      ...
      scriptProcess.stdout.on('data', (data) => {
        ...
        scriptInfo.output.push({ type: 'stdout', data: output, timestamp: Date.now() });
      ...
      scriptProcess.stderr.on('data', (data) => {
        ...
        scriptInfo.output.push({ type: 'stderr', data: output, timestamp: Date.now() });
      ...
      scriptProcess.on('close', (code) => {
        ...
        if (code === 0) {
          this.logger.info('Script completed successfully', { id, name, duration });
          resolve(result);
        } else {
      ...
      scriptProcess.on('error', (error) => {
        this.runningScripts.delete(id);
        this.logger.error('Script process error', { id, name, error: error.message, stack: error.stack });
  ...
  async listScripts() {
    try {
      ...
      this.logger.error('Failed to list scripts', { error: error.message, directory: this.scriptDirectory });
      return [];
    }
  }
  ...
  getRunningScripts() {
    return Array.from(this.runningScripts.values()).map(({ id, name, startTime }) => ({
      id,
      name,
      startTime,
