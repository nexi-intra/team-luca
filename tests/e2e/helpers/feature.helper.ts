...
export class FeatureHelper {
  ...
  async waitForFeature(featureId: string, options?: { timeout?: number }) {
    await this.page.waitForSelector(`[data-feature-id="${featureId}"]`, options);
  }

  async expectFeatureEnabled(featureId: string) {
    const element = await this.page.waitForSelector(`[data-feature-id="${featureId}"]`);
    const opacity = await element.evaluate((el) => window.getComputedStyle(el).opacity);
    const pointerEvents = await element.evaluate((el) => window.getComputedStyle(el).pointerEvents);
